...
(function (global, setting) {
    ...
    var image_box = {
        ...
        prev: KStyle.create("div", {
            class: "ks-prev"
        }),
        next: KStyle.create("div", {
            class: "ks-next"
        }),
        ball: KStyle.create("div", {
            class: "ks-ball"
        })
    };
    ...
    KStyle.lazy = function (elements, bg) {
        ...
        if (global.IntersectionObserver) {
            ...
            KStyle.each(elements, function (item) {
                ...
                if (!item.getAttribute("ks-lazy")) list.push({
                    el: item,
                    link: item.link
                });
            });
            ...
            if (request.readyState === 4) {
                if (request.status === 200 || request.status === 304) {
                    if (prop.type) {
                        switch (prop.type) {
                            case "text":
                                prop.success(request.responseText);
                                break;
                            case "json":
    ...
    console.log("%c Nandisec %c http://ruo.me ", "color: #fff; margin: 1em 0; padding: 5px 0; background: #3498db;", "margin: 1em 0; padding: 5px 0; background: #efefef;");
